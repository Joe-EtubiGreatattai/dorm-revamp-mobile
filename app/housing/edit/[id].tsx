...
export default function EditProperty() {
    ...
    useEffect(() => {
        const fetchListing = async () => {
            console.log('🏠 [EDIT] Fetching listing:', id);
            try {
                ...
                setFormData({
                    title: data.title || '',
                    category: data.category || 'Self-Con',
                    price: formattedPrice,
                    address: data.address || '',
                    description: data.description || '',
                    amenities: data.amenities || [],
                    images: data.images || [],
                    tourFee: data.tourFee ? data.tourFee.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',') : '',
                });
                console.log('✅ [EDIT] Form data loaded');
            } catch (error: any) {
                console.error('❌ [EDIT] Failed to load listing:', {
                    error,
                    message: error?.message,
                    response: error?.response?.data,
                    status: error?.response?.status
                });
                setErrorMessage(error?.response?.data?.message || 'Failed to load listing details.');
    ...
    const handleNext = async () => {
        if (currentStep < STEPS.length - 1) {
            ...
            try {
                ...
                const tourFeeNum = formData.tourFee ? parseFloat(formData.tourFee.replace(/,/g, '')) : 0;
                ...
                await housingAPI.updateListing(id as string, {
                    ...formData,
                    price: priceNum,
                    tourFee: tourFeeNum,
                });
                        ...
                        <View style={styles.inputGroup}>
                            ...
                            {formData.tourFee && formData.price && (
                                <Text style={[
                                    styles.feeNote,
                                    {
                                        color: (parseFloat(formData.tourFee.replace(/,/g, '')) > parseFloat(formData.price.replace(/,/g, '')) * 0.05)
                                            ? '#ef4444'
                                            : colors.subtext
                                    }
                                ]}>
                                    Max allowed: ₦{(parseFloat(formData.price.replace(/,/g, '')) * 0.05).toLocaleString()} (5% of rent)
                                </Text>
                            )}
                        </View>
                    ...
                    <Animated.View entering={FadeInRight} exiting={FadeOutLeft} style={styles.stepContainer}>
                        ...
                        <View style={styles.infoBox}>
                            ...
                            <Text style={[styles.infoText, { color: colors.subtext }]}>
                                Your changes will be reviewed by our team before going live. This typically takes 2-4 hours.
                            </Text>
                        </View>
                    </Animated.View>
                );
    ...
    if (isLoading) {
        return (
            <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]} edges={['top']}>
                ...
                <CustomLoader message="Loading listing..." />
            </SafeAreaView>
        );
    }
    ...
    return (
        <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]} edges={['top']}>
            ...
            {/* Premium Header */}
            <View style={[styles.premiumHeader, { backgroundColor: colors.background }]}>
                <TouchableOpacity onPress={handleBack} style={styles.headerBtn}>
                    <Ionicons name="arrow-back" size={24} color={colors.text} />
                </TouchableOpacity>
                <View style={styles.headerTitleContainer}>
                    <Text style={[styles.headerTitle, { color: colors.text }]}>Edit Listing</Text>
                    <Text style={[styles.headerSubtitle, { color: colors.subtext }]}>{formData.title || 'Property'}</Text>
                </View>
                <View style={{ width: 40 }} />
            </View>
